...
- **Trigger**: when the element is clicked, or scrolls into view.
- **Frequency**: every time, once per session, once per visitor, or with a
  cooldown.
...
- **Test** runs class actions against the canvas immediately and explains
  what the other actions will do on the live site.
...
- **Hamburger menu**: on the button, *When clicked → Toggle class
  `menu-open`* targeting your nav container; style `.menu-open` in the
  theme.
