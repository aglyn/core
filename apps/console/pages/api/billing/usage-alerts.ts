...
export default async function handler(
  ...
  if (!cronSecret) {
    return res
      .status(501)
      .json({ error: 'Usage alerts are not configured (CRON_SECRET).' })
  }
  ...
  try {
    ...
    return res.status(200).json({ alerted: alerted.length, details: alerted })
  } catch (error) {
    console.error(error)
    return res.status(500).json({ error: 'Usage alert run failed' })
  }
}
