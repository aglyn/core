...
import { isSiblingNameTaken, normalizeFolderName } from '@aglyn/aglyn'
...
export default async function handler(
  ...
  try {
    ...
    if (action === 'rename') {
      ...
      const name = normalizeFolderName(String(req.body?.name ?? ''))
      if (!folderId || !name) {
      ...
      return res.status(200).json({ ok: true, moved: assets.length })
    }
    ...
    if (action === 'move-assets') {
      const mediaIds = Array.isArray(req.body?.mediaIds)
        ? (req.body.mediaIds as unknown[]).map(String).slice(0, 100)
        : []
      const folderId = req.body?.folderId ? String(req.body.folderId) : null
      if (!mediaIds.length) {
        return res.status(400).json({ error: 'Missing mediaIds' })
      }
      let moved = 0
    ...
    return res
      .status(500)
      .json({ error: error?.message ?? 'Folder operation failed' })
  }
}
