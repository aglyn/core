 ...
 */
import {firebaseAdmin} from '@aglyn/core-data-tenancy'
import {
  createHttpRefCode,
  HttpRefCodeSimple,
  HttpRefCodeSubject,
...
async function users(request: NextApiRequest, response: NextApiResponse) {
  const {query: {nextPageToken}} = request
  let data = null
  let error = null
  ...
  if (error || data.error) {
    ...
    return nextHandleJsonResponse(
      ...
      err?.['code'] || err?.['statusCode'] || HttpStatusCode.INTERNAL_SERVER_ERROR,
      {
        ...
        statusMessage: err?.['message'] || err?.['statusMessage'] || HttpRefCodeSimple.INVALID_REQUEST,
        error: err,
