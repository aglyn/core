...
const mapIconData = require('./utils/map-icon-data')
...
const dir = (name) => `${outDir}${version}/${name}/`
...
function writeModuleFiles() {
  // Generate default exported module files
  generateFile({data: icons, file: `mdi-icons-array`, dir: dir('modules'), type: 'module'})
  icons.forEach((icon) => {
    generateFile({data: icon, file: `mdi-${icon.id}`, dir: dir('modules/isolated'), type: 'module'})
  })
}

function writeNamedFiles() {
  // Generate named module files
  generateFile({data: icons, file: 'mdi-icons-named', dir: dir('modules'), type: 'named'})
}
